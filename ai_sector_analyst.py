...
try:
    ...
    # 2. 깊이 있는 분석을 유도하는 프롬프트 (분량 제한 해제)
    prompt = f"""
    ...
    - 제목의 종목 코드는 반드시 6자리 숫자로 괄호 안에 표기할 것. (예: 삼성전자 (005930))
    ...
    # 4. 💡 [핵심 패치] 종목 코드 추출 및 네이버 금융 일봉 차트 이미지 가져오기
    chart_html = ""
    # 정규식을 이용해 괄호 안의 6자리 숫자(종목코드)를 찾아냄
    match = re.search(r'\((\d{6})\)', ai_report_md) 
    if match:
        stock_code = match.group(1)
        # 네이버 금융의 실시간 캔들 차트 이미지 URL
        ...
        """
        print(f"✅ 일봉 차트 준비 완료! (종목코드: {stock_code})")

    # 5. 마크다운 -> HTML 변환 (차트 첨부)
    html_content = markdown.markdown(ai_report_md)
    
    # 가독성이 좋았던 '처음 버전'의 CSS 디자인 원상 복구
    css_style = """
    <style>
    ...
    <body>
        {html_content}
        {chart_html} <div class="footer">본 리포트는 OMAKASE AI 퀀트 시스템에 의해 자동 생성된 투자 참고용 자료입니다. (생성일: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')})</div>
    </body>
    </html>
    """

    # 6. HTML을 PDF 파일로 굽기 (넉넉한 여백 0.7in 유지)
    ...
    if not TELEGRAM_BOT_TOKEN or TELEGRAM_BOT_TOKEN == "여기에_로컬테스트용_토큰입력":
        ...
        with open(pdf_filename, 'rb') as pdf_file:
            ...
            data = {'chat_id': TELEGRAM_CHAT_ID, 'caption': "📊 오늘의 [오마카세 리서치] 딥 리서치 PDF (일봉 차트 포함)가 도착했습니다!"}
