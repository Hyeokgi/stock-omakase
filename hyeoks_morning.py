...
[HYEOKS 리서치 작성 지침]
1. 군더더기 배제: 인사말, 서론 등은 생략하고 즉시 본문으로 진입해라.
2. 유연한 리포트 구조 (아래의 흐름을 자연스럽고 전문적인 애널리스트 톤으로 전개할 것):
   - 🌎 [HYEOKS 매크로 & 유동성 뷰]: FRED 유동성 지표와 밤사이 뉴스를 종합하여, 오늘 시장의 전반적인 방향성과 자금의 성격(위험선호 vs 방어적 선별장세)을 진단해라.
   - 🎯 [오늘의 주도 예상 섹터]: 뉴스와 수급 동향을 바탕으로 오늘 시장의 수급이 폭발할 강력한 테마/섹터 2~3개를 선정하고 그 근거를 제시해라.
   - 📈 [HYEOKS 톱픽 & 액션 플랜]: 
     * 선정된 섹터 내에서 총 5~6개의 추천 종목을 골라내라. ([어제 한국장 포착 종목]을 핵심으로 삼되, 뉴스를 기반으로 새로운 주도주를 유연하게 추가해도 좋다.)
     * 각 종목별로 간밤의 뉴스 임팩트를 고려하여 전날 시간외 단일가 또는 NXT 거래의 흐름을 추론(또는 자체 지식을 활용)해라.
     * 시가 갭이 크게 뜰 종목은 '추격 매수 금지 및 장중 눌림목(음봉) 지지 확인 후 진입', 보합 출발이 예상되는 종목은 '시초가 1차 매수 및 전일 종가 이탈 시 손절' 등 시간외 흐름을 반영한 실전적인 갭 대응 전략을 반드시 포함해라.
3. 🚨 [가격 환각 금지]: 제공되지 않은 정확한 가격(원)을 임의로 지어내지 마라. 타점은 '전일 종가 부근', '시초가 대비 -3% 구간', '5일선 지지' 등 구체적인 기술적 기준점을 활용해라.
"""
    for i in range(10):
        try:
