...
# 🚨 [18시 시간외 보강 완료] output1, output2 유연 구조 및 수치 문자열 콤마 제거 필터 장착
def fetch_extra_closing_prices_from_kis(code):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            price_str = str(output.get("stck_prpr", "0")).replace(",", "")
        ...
        if res_fb.status_code == 200:
            ...
            price_fb_str = str(output_fb.get("stck_prpr", "0")).replace(",", "")
...
def analyze_single_stock(name, code, is_warning_market, theme_rank_dict, all_theme_map, kospi_rate, past_theme_map, static_db, theme_historical_max, long_term_stocks):
    ...
    try:
        ...
        # 🚨 [20시 NXT 단일가 입체적 탐색 가동] 3대 키 프레임 교차 복합 검증 파싱
        ...
        # --------------------------------------------------
        # STEP 3: 파생 변수 계산 및 [전고 변수 가공 라인 완벽 복원 보장]
        # --------------------------------------------------
        ...
        # 💡 [무결성 복원] 실전 연산 시 NameError를 제거하기 위해 전고점 캐시 필드 정밀 정의 완료
        ...
        # --------------------------------------------------
        # 🚨 STEP 8: [프로그램 수급 크롤러 완벽 고도화 적용 및 비중 판정식 이식]
        # --------------------------------------------------
                ...
                if len(cols) >= 6:
                    ...
                    if first_col and (':' in first_col or '.' in first_col or first_col[0].isdigit()):
                        ...
                        if clean_amt and clean_amt not in ('', '-', '.'):
                            ...
                            if trading_value > 0:
                                # 💡 절대금액 한계 극복 -> 거래대금 대비 프로그램 순매수 비중 공식 가동
        ...
        # 💡 [A급 신호 면책 진입 전략 이식] 외인 5일 누적 30억 이상 + 전고 대비 -15% 이내 눌림목 허용
        ...
        # 💡 [가산점 퀀트 로직 최적화 이식] 시간외 단일가 폭발(+2% 이상) 및 NXT 우위 유입점 가중 반영
        if krx_rate >= 2.0:
            base_score += 15
        ...
        # 🚨 [수급 분리 패치] 기관 및 외인의 절대 수집액을 억 단위 부호 포맷으로 정밀 분리
        i_sign = "+" if acc_i_buy_eok > 0 else ""
        f_sign = "+" if acc_f_buy_eok > 0 else ""
        i_col_val = f"{i_sign}{acc_i_buy_eok:.1f}억" if acc_i_buy_eok != 0 else "0억"
        f_col_val = f"{f_sign}{acc_f_buy_eok:.1f}억" if acc_f_buy_eok != 0 else "0억"

        # 정확히 31차원의 단일 행 데이터셋 생성 구조 도출
        result_row = [
            ...
            program_text, int(display_high_250d), 
            i_col_val, f_col_val, # 기관(22) / 외인(23) 독립분리 완료
        ...
        print(f"❌ 분석 엔진 예외 가드 강제 통과 [{name}]: {e}")
        return fail_fallback, None
...
def update_technical_data(df_theme, all_theme_map):
    try:
        ...
        if is_reset_time:
            ...
            static_sheet.batch_clear(['A2:F2000'])
        else:
            try:
        ...
        # 💡 [정렬 보안 패치 완료] 반환 데이터가 온전한 31차원 구조(index 30 존재)일 때만 스코어 역순 정렬 처리 수행
        results.sort(key=lambda x: x[30] if (x and len(x) > 30) else 0, reverse=True)
            ...
            if not is_reset_time:
                for row in old_data[1:]:
                    if len(row) > 15:
                        ...
                        # 하이브리드 가변 인덱싱 안전 매핑 가드
                        tgt_idx = 15 if len(row) > 15 else 14
                        stp_idx = 16 if len(row) > 16 else 15
                        if any(key in briefing for key in ["리포트 발송 완료", "간단 브리핑"]) and "계산중" not in str(row[tgt_idx]):
        ...
        for r in results:
            ...
            if c_code in existing_data:
                # 31차원 구조에 따른 슬라이싱 매핑 수정
                r[24] = existing_data[c_code]["target"]
                r[25] = existing_data[c_code]["stop"]

        try:
            helper_sheet = doc.worksheet("주가데이터_보조")
        except:
            helper_sheet = doc.add_worksheet(title="주가데이터_보조", rows="150", cols="30")

        # 💡 [정밀 독립 분리 30컬럼 헤더 레이아웃 구성]
        extended_headers = [
            "종목명", "종목코드", "현재가", "등락률", "5일평균", "20일평균", "거래량비율", "AI신호",
            "마스터타점", "브리핑상태", "당일고가", "당일저가", "60일고가", "시가총액", "캔들상태",
            "전고거리", "20일이격", "대장구분", "거래과열", "테마명", "프로그램", "52주고가",
            "기관 누적(5일)", "외인 누적(5일)", "목표가(AI)", "손절가(AI)", "종목쿼터", "시간외단일가(18시)", "NXT야간종가(20시)", "장구분"
        ]

        # 🚨 [무결성 쓰기 락 해제] 인덱스 30번까지의 순수 시각 데이터를 batch_clear 이후 밀어넣기 처리 보장
        helper_sheet.batch_clear(['A1:AD1500'])
        helper_sheet_data = [extended_headers] + [r[:30] for r in results]
        helper_sheet.update(range_name="A1", values=helper_sheet_data, value_input_option="USER_ENTERED")
        print(f"✅ 총 {len(results)}개 종목 판독 완료 (주가데이터_보조 기관/외인 분리 갱신 동기화 완료)")

        # ============================================================
        # 하이브리드 독립 풀(Pool) 선별 게이트 (A~T열 20컬럼 분리 포맷팅 정렬 완화)
        # ============================================================
        ...
        for r in results:
            ...
            if any(kw in tajeom for kw in scanner_keywords):
                ...
                # 💡 [스캐너 독립 분리 배치] 기관과 외인을 별도로 분리하여 총 20차원 행으로 확장 빌드
                row_data = [
                    하이퍼링크, 시장구분, f"'{종목코드}", 현재가, 등락률, 테마명, AI신호, 거래량비율,
                    tajeom, ai_briefing, 스코어, 프로그램, 고가_52주, 기관누적, 외인누적, ai_target, ai_stop,
                    r[27], r[28], r[29]                      
                ]
        ...
        db_scanner_sheet = doc.worksheet("DB_스캐너")
        db_scanner_sheet.batch_clear(['A2:AT1000'])
        if top_20_results:
            db_scanner_sheet.update(range_name="A2", values=top_20_results, value_input_option="USER_ENTERED")
        print(f"🎯 DB_스캐너 {len(top_20_results)}개 전송 완료 (기관/외인 독립 쿼터 정렬 안착 완료)")
...
if __name__ == "__main__":
    ...
    scope_main = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    gc_main = gspread.authorize(ServiceAccountCredentials.from_json_keyfile_name("secret.json", scope_main))
    doc_main = gc_main.open_by_url(SHEET_URL)
    ...
    if now_kst.hour == 15 and 0 <= now_kst.minute <= 50:
        try:
            posted_data = doc_main.worksheet("리포트_게시").get_all_values()
