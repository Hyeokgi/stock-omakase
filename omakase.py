...
def check_warning_market():
    """
    하락장 판단 (셋 중 하나라도 해당하면 경고)
    1. 코스닥 현재가 < 20일 이평 (기존)
    2. 코스닥 5일 이평 < 20일 이평 (추세 꺾임 조기 감지)
    3. 코스피 당일 등락률 < -1.0% (대형 충격일 즉시 방어)
    """
    ...
    try:
        ...
        if len(prices) == 20:
            ma20 = sum(prices) / 20
            ma5  = sum(prices[:5]) / 5
            if prices[0] < ma20: warning_count += 1   # 조건1: 현재가 < 20일선
            if ma5 < ma20:       warning_count += 1   # 조건2: 5일선 < 20일선 (추세 꺾임)
    except:
        pass
    try:
        kospi_rate = get_kospi_fluctuation_rate()
        if kospi_rate <= -1.0: warning_count += 1     # 조건3: 코스피 -1% 이상 급락일
    except:
        pass
...
def analyze_single_stock(name, code, is_warning_market, theme_rank_dict, all_theme_map, kospi_rate, past_theme_map, static_db, theme_historical_max, long_term_stocks):
    ...
    try:
        ...
        try:
            ...
            # 종목별 변동성(ATR)에 따라 Q/R 동적 조정
            # 변동성 높을수록 Q↑(필터 민감), R↑(측정 신뢰도↓)
        ...
        try:
            ...
            for r_tag in rows:
                ...
                if len(cols) >= 7 and cols[0].text.strip().replace('.', '').isdigit():
                    ...
                    acc_f_buy_won += f_buy_won   # ← 외인 누적 (supply_status_col에 필요)
                    valid_days += 1
        ...
        # 외인 액티브 매수: 프로그램 매도에도 불구하고 외인 실물 매집 중
        is_foreigner_active_buy = (f_buy_eok >= 15) and (pg_amount_eok <= 5) and ((f_buy_eok - pg_amount_eok) >= 10)
        ...
        # 종가베팅(M-1 눌림목): 신정재 트레이더 전략
        # 거래량이 죽고 이평선이 수렴하며 전고점 부근에서 대기하는 종목
        is_jongbe_cand = (
            not is_upper_limit
            and not is_long_shadow
            and not is_fatal_drop
            and not is_accumulation_cand       # 모아가기와 중복 방지
            and is_converging                   # 이평선 수렴
            and vol_ratio_yest <= 80            # 전일 대비 거래량 감소
            and vol_ratio_10d  <= 70            # 10일 평균 대비도 감소
            and current_price  >= ma20          # 20일선 위 (추세 유지)
            and is_near_high                    # 60일 전고점 90% 이내
            and trading_value  >= min_nulim_tv  # 최소 거래대금 충족
        )

        # 하락장 역행 강세: 코스피 하락일에 오히려 상승하는 진성 강세주
        is_relative_strong = (kospi_rate <= -0.5) and (change_rate >= 0.02) and not is_fatal_drop
        ...
        elif is_jongbe_cand: signal = "🎯 M-1 눌림목 (종베)" + supply_text
        elif is_relative_strong: signal = "💪 하락장 역행 강세" + supply_text
        ...
        if is_fatal_drop:
            ...
            tajeom_multiplier = 1.3
        elif is_relative_strong:
            master_tajeom_base = "💪 [관심/수급] 하락장 역행 강세 (진성 강세주)"
