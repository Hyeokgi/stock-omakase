...
오후 7:30까지 무료 메시지를 모두 사용했습니다.
더 얻기
주가데이터 탭 오류 및 프로그램 전체 수정
마지막 메시지 21분 전
현재의 omakase.py 코드인데 주식정리 구글시트의...
마지막 메시지 4일 전
칼만 필터 기반 비밀 이동평선의 효용성
마지막 메시지 5월 21일
제미나이 주식 자동 추천 프로그램 실시간 데이터 반영 문제
마지막 메시지 5월 10일
한국투자증권 API로 구글 시트에 주식 데이터 연동
마지막 메시지 5월 10일
메모리
나만
Purpose & context Hyeoks is a salaried professional building and operating a sophisticated automated Korean stock market intelligence system called HYEOKS. The goal is a high-probability, stable-performing system for both bull and bear markets, delivering actionable trading recommendations via Telegram (including PDF reports) for short-term swing, closing-price betting (종가베팅), and medium-term strategies. Trading philosophy synthesizes approaches from three Korean educators: 쥬쥬총회 (momentum/gap trading), 신정재 (program flow/leading stock identification), and 강창권 (safety margin/pattern trading). --- Current state The core system file is omakase.py (Korean stock scanner), with supporting scripts hyeoksanalyst.py (AI briefing engine), hyeoksmorning.py (morning briefing), scannerbot.py (closing-price sniper bot). The system runs on GitHub Actions (paid plan), outputs to Google Sheets, and integrates KIS OpenAPI, Naver Finance, Gemini AI, and Telegram. Multiple versioned iterations of omakase.py have been actively debugged (through at least 20260608). Recently resolved issues: isnear52whigh / isnearhigh used before definition → silent NameError causing all stock analysis to fail Program trading data (프로그램매매현황) always showing neutral → replaced iframe-blocked siseprogram.naver with frgn.naver foreign volume × price proxy KRX after-hours single price (AA column, 18시) not populating → corrected KIS API transaction ID to FHPST02320000 NXT after-hours data (AB column, 20시) using FNPST01010100 with fidcondmrktdivcode: "N" Duplicate STEP 8 block removed; missing accfbuywon accumulation fixed livesuccess flag referenced before declaration; second-engine fallback had dead-code indentation Gemini model migration: gemini-1.5-flash → gemini-2.5-flash (simple briefings) + gemini-2.5-pro (main reports) getmarketcap() bug: string like "1조 2,345" passed directly to int() always returned 0 → fixed parsing GitHub token moved from GAS hardcode to PropertiesService (requires manual one-time setupGitHubToken() run) hyeoksmorning.py has if True: hardcoding that disables the Telegram send path — flagged but status unclear System architecture highlights: Kalman filter-based trend phase classification with ATR-based dynamic Q/R parameters Historical DNA filtering (high trading value thresholds for identifying leading stocks) Multi-condition bear market detection (코스닥 5일선 < 20일선 + 코스피 -1% trigger) Dual institutional/foreign buying detection (쌍끌이) DB중장기 core portfolio integration Multi-threaded parallel processing (~30–60 stocks) Backtesting log with T+1/T+3 return tracking KIS API token caching in Google Sheets 종가베팅 M-1 pullback entry signal (isjongbecand) Bear market relative strength flag (isrelativestrong) GAS scheduler running multiple times daily --- On the horizon Gaps vs. institutional systems: Claude assessed the system as top 1% among individual investors but identified remaining gaps: no position sizing logic, no post-entry management automation, no portfolio correlation/theme concentration management, binary (vs. four-phase) market regime classification, and backtesting feedback loop not yet actively used After-hours data reliability: KRX 18:00 and NXT 20:00 data collection has been technically resolved in code but real-world reliability (especially GitHub Actions timing variance) remains a known concern True theme leader detection: Beyond simple volume ranking — a concrete improvement Hyeoks expressed strong interest in Backtest loop activation: Using real T+1/T+3 return data to validate and tune quant score weightings --- Key learnings & principles Naver mobile API (overMarketPriceInfo) returns only the currently active after-hours session — KRX and NXT data cannot be retrieved simultaneously GitHub Actions environments block certain iframe-based Naver pages (e.g., siseprogram.naver); direct JSON APIs or alternative endpoints are required Silent failures are the dominant failure mode: NameErrors and undefined variables don't surface loudly but corrupt all downstream output Both Gemini and Claude can produce confident but incorrect code; Hyeoks actively cross-checks and values honest assessment over optimistic reassurance PropertiesService setup in GAS requires a manual one-time initialization run before automation can take over Gemini model versions must be verified for availability — deprecated/unsupported model names cause silent or confusing failures batchupdate / batchclear should be preferred over individual cell calls to avoid API quota exhaustion --- Approach & patterns Iterative versioned file development: keeps dated copies of omakase.py as checkpoints Actively uses both Claude and Gemini AI in parallel, comparing outputs and noting each AI's failure patterns Prefers direct, honest technical assessment — explicitly dislikes overconfident reassurance Debugs by sharing full code sections; tends to apply suggested fixes and report back with new observed behavior Has produced internal documentation (HTML master guidebook with SVG charts) for the system --- Tools & resources APIs: KIS OpenAPI (Korea Investment Securities), Naver Finance (mobile JSON + HTML crawling), Naver mobile stock API Infrastructure: GitHub Actions (paid), Google Sheets, Google Apps Script (GAS) AI: Gemini 2.5 Flash + 2.5 Pro, Claude Delivery: Telegram (PDF reports + real-time alerts) Other: AppSheet (referenced in system architecture)
...
Claude 메시지 한도에 도달했습니다. 제한이 오후 7:30에 재설정됩니다. 더 높은 제한을 원하시면 Pro 플랜을 살펴보세요.
