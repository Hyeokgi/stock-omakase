    ...
    if current_hour != 15:
        ...
        for i, row in enumerate(db_rows[1:], start=2):
            ...
            if len(row) > 9 and "리포트 발송 완료" not in str(row[9]):
                ...
                💡 [시간대별 실전 지침 및 가격 설정 룰]
                - 위 '파이썬 시스템 권장 목표가/손절가'는 이동평균선(5일/20일선)과 60일 전고점 저항을 수학적으로 계산한 매우 정확한 지표입니다. 
                - 브리핑을 작성할 때 반드시 이 시스템 권장 가격을 최우선으로 참고하여 최종 "target_price"와 "stop_loss" 수치를 도출하십시오. 임의의 좁은 비율(예: -3%)로 억지 손절가를 만들지 마십시오.
                - 현재 시간(KST {current_hour}시)에 맞는 대응 전략을 1~2문장(70자 내외)으로 작성하십시오.
