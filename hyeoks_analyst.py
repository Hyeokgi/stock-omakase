    ...
    if current_hour != 15:
        ...
        for i, row in enumerate(db_rows[1:], start=2):
            if len(row) > 9 and "리포트 발송 완료" not in str(row[9]):
                ...
                ■ 🤖 [시스템 산출 1차 목표가]: {row[14] if len(r) > 14 else ''}
                ■ 🤖 [시스템 산출 1차 손절가]: {row[15] if len(r) > 15 else ''}
                
                💡 [손익비(Risk/Reward) 및 가격 설정 절대 원칙 (현재 KST 시간: {current_hour}시)]
                1. 타점의 성격 파악: '시크릿 1차 파동', '박스권 탈출' 등 상승 추세의 초입이라면, [시스템 산출 1차 목표가]를 참고하되 기대 수익률을 +15% ~ +30% 이상으로 과감하게 열어두십시오.
                2. 🚨 손익비 강제 룰: 목표가 수익률과 손절가 하락률이 1:1이 되는 것은 최악의 매매입니다. 반드시 "손절은 짧게(-3~-5%), 수익은 길게(+10~+30%)" 가져가는 1:2 이상의 압도적인 손익비로 가격을 세팅하십시오.
                3. 시간대 룰: 11시~13시는 가짜 돌파 주의, 14시~15시는 종가베팅 수급 유입에 초점을 맞추십시오.
    ...
    for i, r in enumerate(latest_db_data[1:], start=2):
        if len(r) > 9:
            ...
            if "리포트 발송 완료" not in str(r[9]):
                ...
                ■ 타점 위치: {r[8] if len(row) > 8 else ''}
                ■ 당일 수급: {r[11] if len(row) > 11 else ''}
                ■ 52주 고가: {r[12] if len(row) > 12 else ''}
                ■ 테마: {r[5] if len(row) > 5 else ''}
                ■ 🤖 [시스템 산출 1차 목표가]: {r[14] if len(r) > 14 else ''}
                ■ 🤖 [시스템 산출 1차 손절가]: {r[15] if len(r) > 15 else ''}
