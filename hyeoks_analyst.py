...
try:
    ...
    def generate_hyeoks_report(st_type):
        ...
        # 💡 [핵심 패치] 다각도 심층 분석 추가 + 1종목 2페이지 분량 강제 프롬프트
...
1. 분량 및 깊이 강제 (1종목당 2페이지 분량): 과거 심층 리포트처럼 해당 기업의 숨겨진 모멘텀, 업황, 타 종목과의 비교 등을 매우 상세하게 서술해라. 각 섹션마다 전문가적 식견을 길고 깊이 있게 풀어내어, PDF 변환 시 정확히 1종목당 2페이지 분량이 나오도록 텍스트 볼륨을 극대화해라.
...
[출력 양식 (마크다운 유지, 각 섹션별로 논리적이고 매우 긴 분량으로 서술할 것)]
...
(기업의 펀더멘털, 영위하는 핵심 사업, 모멘텀을 상세히 설명. 5문장 이상으로 깊이 있게 요약.)
</div>

## 1. 펀더멘털 및 매크로 테마 주도력 심층 고찰
(여의도 기관 리포트 수준의 깊이. 현재 거시경제 상황 속에서 해당 종목이 속한 섹터의 턴어라운드 가능성, 신사업 모멘텀, 동종 업계 타 종목 대비 비교 우위 등을 다각도로 매우 상세하게 서술할 것. 분량을 충분히 확보할 것.)

## 2. 시각적 차트 판독 및 거래량 딥리딩
(과거 주요 매물대, 이평선의 이격도, 장대양봉/음봉의 의미, 최근 거래량 증감의 특징을 시각적 관점에서 아주 정밀하게 해부할 것. 특정 날짜나 가격대를 짚어가며 상세히 설명할 것.)

## 3. 실전 타점 시나리오 및 리스크 관리 전략
(위에서 지시한 '실전 야수의 타점 설계' 원칙 적용. 1차/2차 진입 가격대, 오전 투매 대기 여부, 갭상승/하락 시의 플랜 A/B 시나리오를 구체적으로 서술. 목표가 산출 근거와 칼 같은 손절가 기준을 논리적으로 제시할 것.)
"""
