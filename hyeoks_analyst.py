...
try:
    ...
    if current_hour != 15:
        ...
        for i, row in enumerate(db_rows[1:], start=2):
            if len(row) > 9 and "리포트 발송 완료" not in str(row[9]):
                ...
                ■ 타점 위치(배지): {row[8] if len(row) > 8 else ''}
                ■ 당일 수급: {row[11] if len(row) > 11 else ''}
                ■ 52주 고가: {row[12] if len(row) > 12 else ''}
                ■ 테마: {row[5] if len(row) > 5 else ''}
                ■ 🤖 [시스템 1차 산출가]: 목표가 {row[14] if len(row) > 14 else ''} / 손절가 {row[15] if len(row) > 15 else ''}
                
                💡 [AI 자율 가격 산출 및 손익비(Risk/Reward) 최적화 룰]
                1. '시스템 1차 산출가'는 기계적인 참고값일 뿐입니다. 당신은 제시된 '타점 위치(배지)'와 '현재가'의 맥락을 분석하여 목표가와 손절가를 자율적으로 재조정하십시오.
                2. 기계적인 1:1 손익비를 절대 지양하십시오. 당신의 통찰력으로 유의미한 지지/저항 라인을 유추하여, 트레이딩에 적합한 손익비(최소 1:1.5 ~ 1:3 이상)를 창출해야 합니다.
                3. 타점별 맞춤 대응: 
                   - '시크릿 1차 파동' 등 추세 초입의 배지가 붙었다면, 상단 매물대가 열려있다고 판단하여 목표가를 과감히 높이십시오.
                   - '박스권 탈출'이나 '신고가 턱밑'이라면, 돌파 라인을 지지선(손절가)으로 매우 타이트하게 올려잡아 리스크를 최소화하십시오.
    ...
    # ==========================================
    # 💡 [V11.5 하이브리드 로직] pick_prompt 업데이트
    # ==========================================
    pick_prompt = f"""
    ...
    for i, r in enumerate(latest_db_data[1:], start=2):
        if len(r) > 9:
            ...
            if "리포트 발송 완료" not in str(r[9]):
                ...
                ■ 종목명: {stock_name}
                ■ 현재가: {r[3] if len(row) > 3 else ''}
                ■ 타점 위치(배지): {r[8] if len(row) > 8 else ''}
                ■ 당일 수급: {r[11] if len(row) > 11 else ''}
                ■ 52주 고가: {r[12] if len(row) > 12 else ''}
                ■ 테마: {r[5] if len(row) > 5 else ''}
                ■ 🤖 [시스템 1차 산출가]: 목표가 {r[14] if len(row) > 14 else ''} / 손절가 {r[15] if len(row) > 15 else ''}
