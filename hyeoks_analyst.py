...
try:
    ...
    def generate_hyeoks_report(st_type):
        ...
        # ==========================================
        # 📝 [리포트 작성 프롬프트] 들여쓰기 완벽 제거 및 좌측 정렬
        # ==========================================
        if st_type == "short":
            final_prompt = f"""너는 전설적인 실전 트레이더들의 매매 기법을 체화한 HYEOKS 증권의 최고 수석 퀀트 애널리스트야. 
...
1. 은어 배제: 'SS급', 'S급', '단타용' 등 시스템 이모지 및 은어를 리포트 본문에 절대 노출하지 말 것.
...
"""
            ...
            final_prompt = f"""너는 전설적인 실전 트레이더들의 매매 기법(거래량 씨마름, VCP 등)을 체화한 HYEOKS 증권의 최고 수석 퀀트 애널리스트야. 
