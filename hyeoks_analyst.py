...
확인되지 않은 구체적 수치나 사실을 지어내지 말고, 일정의 성격과 일반적으로 알려진 배경지식에 기반한 분석만 작성하십시오."""
    ...
    [🆕 세 픽의 "정상적으로 예상되는 빈도"가 서로 다릅니다 — 반드시 숙지하십시오]
    단기·중기·장기는 완전히 독립적인 판단이며, 매일 3개를 다 채워야 한다는 부담을 갖지 마십시오.
    오히려 다음과 같은 빈도가 정상입니다:
    · 단기: 자금이 몰리는 신규 테마는 거의 매일 있을 수 있어 상대적으로 자주(거의 매일) 나올 수 있습니다.
    · 중기(스윙): 진짜 과매도 반등 조건을 갖춘 종목은 매일 있지 않습니다. 며칠에 한 번, 혹은 그보다 드물게
      나오는 것이 지극히 정상이며, 억지로 찾아내려 하지 마십시오.
    · 장기: 실적과 산업 흐름까지 뒷받침되는 진짜 구조적 성장 후보는 가장 드뭅니다. 몇 주에 한 번, 혹은 그보다
      드물게 나와도 전혀 이상하지 않습니다. "오늘은 장기 후보가 마땅치 않다"는 판단이 훨씬 흔한 정상 상태입니다.
    각 자리는 서로 무관하게 판단하십시오 — 장기 후보가 좋다고 해서 단기·중기를 억지로 채우거나 반대로 비울
    필요가 없고, 단기가 마땅치 않다고 장기 기준을 낮출 필요도 없습니다. 하루에 1개만 나오는 날, 심지어
    셋 다 000000인 날이 있어도 그건 시스템이 정상적으로 작동하고 있다는 뜻입니다.
    ...
    ④ 🚨 중요: "000000"을 반환하는 것은 실패가 아니라 성공적인 판단입니다. 확신이 서지 않는 종목을 억지로 채워 넣는 것보다,
       ...
       이 판단은 단기·중기·장기 각각 완전히 독립적입니다 — 다른 두 자리에서 좋은 후보를 찾았다고 해서
       나머지 자리 기준을 낮추지 마십시오. 오늘 셋 중 하나만 채워지거나, 심지어 셋 다 000000이어도 정상입니다.
