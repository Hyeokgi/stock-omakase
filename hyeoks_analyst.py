...
1. 분량 초정밀 제어 (1종목당 차트 포함 거의 2페이지를 채운 분량): 분석은 깊게 하되, 중복되는 설명이나 늘어지는 문장을 압축해라. 각 섹션의 핵심만 밀도 있게 타격하여, 리포트 하단에 첨부될 '차트 이미지'까지 포함했을 때 정확히 2페이지 분량으로 딱 떨어지도록 텍스트 길이를 절묘하게 맞춰라.
...
# 💡 [신규 추가 패치] 시간외 변동성 및 익일 시가에 따른 '시나리오 대응' 로직
7. 익일 시가 및 갭(Gap) 대응 시나리오: 파라볼릭 슈팅이나 대량 거래 돌파가 발생한 종목은 익일 시가의 위치가 전략의 성패를 결정한다.
   - 갭상승 과다 시: "만약 익일 시가가 +5% 이상 과도하게 갭을 띄워 시작할 경우, 추격 매수 시나리오는 즉시 파기(Invalid)하고 장중 눌림목이나 5일선 지지 여부를 다시 확인하라"는 경고를 반드시 포함해라.
   - 시간외 변동성 고려: "리포트 작성 시점 이후 시간외 단일가 등에서 급등락이 발생할 경우, 제시된 진입 가격은 즉시 무효화되며 시장가 추격보다 하단 지지선 재확인이 우선이다"라는 실전 대응 원칙을 명시해라.   
