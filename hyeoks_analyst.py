    ...
    if current_hour != 15:
        ...
        for i, row in enumerate(db_rows[1:], start=2):
            ...
            if len(row) > 9 and "리포트 발송 완료" not in str(row[9]):
                ...
                - [시간 룰] 만약 현재 시간이 11시~13시(마의 구간)라면: 가짜 돌파(휩쏘) 확률이 높으므로 "오후장까지 관망하며 지지선 이탈 여부 확인"을 권고하십시오.
                - [시간 룰] 만약 현재 시간이 9시~10시(오전장)라면: 당일 주도주의 '오전장 눌림목' 또는 '돌파' 타점을 가장 긍정적으로 평가하십시오.
                - [시간 룰] 만약 현재 시간이 14시~15시(오후장)라면: 내일을 대비하는 '종가베팅' 수급 유입에 초점을 맞추십시오.
