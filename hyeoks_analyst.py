            ...
            if len(row) > 9 and "리포트 발송 완료" not in str(row[9]):
               ...
               💡 [시간대별 실전 지침 및 AI 자율 가격 설정 룰 (현재 KST 시간: {current_hour}시)]
                - [핵심 가격 룰] 위 '파이썬 1차 계산 가격'은 단순 이평선 기반의 참고용 수치입니다. 당신은 실전 트레이더로서 이 수치의 '손익비(Risk/Reward)'와 '변동성'을 평가하여 최종 "target_price"와 "stop_loss"를 독자적으로 결정하십시오.
                  1) 손절가가 현재가와 너무 가깝다면(예: 1~2% 이내), 해당 종목의 변동성을 감안하여 세력의 휩쏘에 당하지 않을 의미 있는 지지선으로 손절가를 넉넉히 낮추십시오.
                  2) [스윙/눌림] 타점인데 목표가(기대수익)가 5% 미만으로 너무 짧아 손익비가 안 나온다면, 다음 매물대 저항선이나 최소 10~15% 이상의 현실적인 스윙 목표가로 새롭게 재설정하십시오.
                - [시간 룰] 11시~13시(마의 구간): 가짜 돌파(휩쏘) 확률이 높으므로 "오후장까지 관망하며 지지선 확인"을 권고하십시오.
                - [시간 룰] 9시~10시(오전장): 당일 주도주의 '오전장 눌림목' 또는 '돌파' 타점을 가장 긍정적으로 평가하십시오.
                - [시간 룰] 14시~15시(오후장): 내일을 대비하는 '종가베팅' 수급 유입에 초점을 맞추십시오.
