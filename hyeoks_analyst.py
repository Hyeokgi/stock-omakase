...
try:
    ...
    for r in tech_data:
        ...
        if re.search(r'매매제한|매수금지|자본잠식|딱지|데이터 부족|3년적자|제외|관망', tajeom_raw): continue 
    ...
    # ==========================================
    # 💡 [V11.6 자율형 AI 로직] pick_prompt (배지 강제 해제 및 자율성 부여)
    # ==========================================
    pick_prompt = f"""
    당신은 대한민국 최고의 주식 트레이더이자 HYEOKS 퀀트 분석가입니다.
    아래는 1차 퀀트 필터링을 통과한 상위 150개 종목 리스트입니다.
    
    단순히 특정 배지나 점수에만 기계적으로 의존하지 마십시오. 제미나이 모델의 강력한 직관과 데이터 융합 추론 능력을 발휘하여, 
    우리의 핵심 매매 철학에 가장 완벽하게 부합하는 '최고의 단기 1종목'과 '스윙 1종목'을 자율적으로 엄선하십시오.

    [핵심 매매 철학 및 AI 자율 선정 가이드]
    1. 단기 슈팅 공략주 (short_term_code): 
       - 당일 강력한 메이저 수급(프로그램 대량유입 등)이 몰리며 전고점 돌파를 목전에 둔 파괴력 있는 주도주를 찾으십시오.
       - '🚀 [당일/단타]', '👑(진성대장)' 등의 배지가 훌륭한 힌트가 될 수 있으나, 이에 얽매이지 말고 퀀트 점수와 에너지 응축 상태를 종합하여 당장 내일 아침 갭상승 확률이 가장 높은 종목을 발굴하십시오.
    
    2. 중기 스윙 공략주 (swing_code) - 쥬쥬총회식 2차 상승 초입 저격 전략: 
       - 1차 파동 이후 시장의 무관심 속에 거래량이 급감하며 의미 있는 지지선(20일선 등)을 지켜내고 있거나, 지루한 박스권을 막 뚫어내며 고개를 드는 '2차 상승의 초입' 종목을 찾으십시오.
       - '🕵️ [세력선 포착]', '🎯 [눌림]', '📦 [박스권 탈출]' 등의 배지 정보를 적극 참고하되, 최종적으로 당신이 판단하기에 하락 리스크는 적고 위로 열린 수익 구간(손익비)이 가장 훌륭한 바닥권/눌림목 자리라면 배지의 유무와 상관없이 과감히 픽하십시오.
       - 🚨 절대 원칙: 단, '🔴 3차 파동', '⚠️ 대시세 고점' 등 이미 시세가 끝물에 달한 과열 종목은 철저히 배제하십시오. 당신은 리스크 관리의 최후 방어선입니다.

    [상위 150개 종목 리스트]
    {pool_str}
    
    [출력 형식]
    반드시 아래 JSON 형식으로만 응답하세요. 없는 사실이나 종목을 지어내지 말고, 오직 위 리스트에 존재하는 종목코드 중에서만 선택하십시오.
    {{
...
(종목 추천 없이 시황과 트레이더의 스탠스만 서술하십시오.)"""
    ...
    # ==========================================
    # 💡 [V11.5 하이브리드 로직] generate_deep_report 업데이트
    # ==========================================
    def generate_deep_report(st_type, best_cand):
        ...
        if st_type == "short":
            ...
            sub_title_prefix = "퀀트-시크릿 하이브리드 스윙 전략"
            strategy_instruction = """
    ...
    try:
        ...
        if not already_logged:
            ...
            for cand in pool_150[:5]:
                ...
                theme_match = re.search(r'테마:(.*?)\|', cand['info']) if '테마:' in cand['info'] else None
                theme_str = theme_match.group(1).strip() if theme_match else "분석대기" # 테마 파싱 추가
                
                log_rows.append([
                    today_str, 
                    cand['name'], 
                    f"'{cand['code']}", 
                    theme_str,          # "수동확인요망" 대신 테마명 입력
                    f"{cand['curr_p']:,}원", 
                    tajeom_str, 
                    f"{cand['score']}점", 
                    "", "" 
