...
def pair_mean(channel, horizon, day):
    stock, _ = COLS[horizon]
    n = pair_count(channel, horizon, day)
    total = f'SUMPRODUCT({mask(channel, day, horizon)}*IFERROR({ref(stock)}*1,0))'
...
def build_rows():
    ...
    rows += [[], ["오늘(수식 기준)", "=TODAY()", VERSION],
             ...
             ["비용", "아래 수익률은 비용 전 고정기간 관측치. 손절/트레일링/슬리피지 미반영."],
    ...
    rows.append(["날짜(달력일)", "단기 추천 행", "단기 짝 표본", "단기 T+5", "차트 T+5", "랜덤 T+5", "단기−차트", "단기−랜덤",
                 "중기 추천 행", "중기 짝 표본", "중기 T+10", "차트 T+10", "랜덤 T+10", "중기−차트", "중기−랜덤"])
