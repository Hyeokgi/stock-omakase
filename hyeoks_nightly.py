...
def main():
    ...
    send_telegram(f"🌙 [HYEOKS 심야 배치 가동]\n\n[시간외 단일가 / NXT 야간거래 / 20일선 / 60일고가 / 신용비율] 수집을 시작합니다.\n(V열 프로그램 데이터는 보존됩니다.)")
    ...
    # 3. 데이터 시트 로드 및 헤더 확장 (X열인 24번째 칸까지 확장)
    ...
    header[20] = "시간외단일가"
    # header[21] = "프로그램(당일)" <- 이 부분은 건드리지 않음
    header[22] = "NXT야간거래"
    header[23] = "신용잔고율"
    all_data[0] = header
    ...
    print(f"📊 총 {len(all_data)-1}개 종목 정밀 스캔 시작")

    for idx in range(1, len(all_data)):
        row = all_data[idx]
        while len(row) < 24: row.append("") # 행 길이 X열까지 맞춤
        ...
        try:
            ...
            res_price = req.get("https://openapi.koreainvestment.com:9443/uapi/domestic-stock/v1/quotations/inquire-price", 
                                headers=kis_headers, params={"fid_cond_mrkt_div_code": "J", "fid_input_iscd": code}, verify=False, timeout=3).json()
            if res_price.get("rt_cd") == "0":
                out = res_price.get("output", {})
                rate = out.get("ovtm_untp_prdy_ctrt", "0")
                prc = out.get("ovtm_untp_prpr", "0")
                if rate and float(rate) != 0:
                    single_val = f"🔺{rate}%" if float(rate) > 0 else f"🔵{rate}%"
                    single_val += f"({int(prc):,}원)"
                else: single_val = "➖0.00%"

            # 💡 [작업 2] NXT 야간 거래 수집 (W열)
            kis_headers["tr_id"] = "FNPST01010100"
            res_nxt = req.get("https://openapi.koreainvestment.com:9443/uapi/domestic-stock/v1/quotations/inquire-nextrade-price", 
                              headers=kis_headers, params={"fid_cond_mrkt_div_code": "J", "fid_input_iscd": code}, verify=False, timeout=3).json()
            if res_nxt.get("rt_cd") == "0":
                out_n = res_nxt.get("output", {})
                n_rate = out_n.get("prdy_ctrt", "0")
                n_prc = out_n.get("stck_prpr", "0")
                if n_rate and float(n_rate) != 0:
                    nxt_val = f"🔺{n_rate}%" if float(n_rate) > 0 else f"🔵{n_rate}%"
                    nxt_val += f"({int(n_prc):,}원)"
                else: nxt_val = "➖0.00%"
            else: nxt_val = "거래없음"

            # 💡 [작업 3] 차트 데이터 (20일선/60일고가)
            kis_headers["tr_id"] = "FHKST03010100"
            res_hist = req.get("https://openapi.koreainvestment.com:9443/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice", 
                               headers=kis_headers, params={"fid_cond_mrkt_div_code": "J", "fid_input_iscd": code, "fid_input_date_1": date_100_days_ago, "fid_input_date_2": today_str_kis, "fid_period_div_code": "D", "fid_org_adj_prc": "0"}, verify=False, timeout=3).json()
            if res_hist.get("rt_cd") == "0":
                h_data = res_hist.get("output2", [])[:60]
